...
"""
8-GPU FLUX模型评估脚本
基于train_flux.py的评估部分，专门用于8卡分布式模型评估
"""
...
def eval_flux_8gpu(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, reward_fn, executor, autocast):
    """8-GPU分布式评估FLUX模型的主函数"""
    ...
    if accelerator.is_main_process:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            results_file = os.path.join(config.save_dir, "eval_results_8gpu.json")
...
def main():
    parser = argparse.ArgumentParser(description="8-GPU FLUX模型评估脚本")
    ...
    parser.add_argument("--save_dir", type=str, default="./eval_results_8gpu", help="结果保存目录")
    ...
    if accelerator.is_main_process:
        ...
        if config.use_wandb:
            try:
                ...
                wandb.init(
                    project="flux-eval-8gpu",
                    name=f"eval_8gpu_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    config={
                        "model_path": config.pretrained.model,
                        "dataset": config.dataset,
                        "prompt_fn": config.prompt_fn,
